...
int* initSets(int graphSize)
{
    int* unionSets = (int*)malloc(graphSize * sizeof(int));
    if (!unionSets)
    {
        return NULL;
    }
...
bool isBipartite(int** graph, int graphSize, int* graphColSize){
    int* unionSets = initSets(graphSize);
    bool ret = true;
    if (!unionSets)
    {
    ...
    for (i = 0; i < graphSize; ++i)
    {
        ...
        for (j = 0; j < graphColSize[i]; ++j)
        {
            ...
            fb = find(graph[i][j], unionSets);
            if (fa == fb)
            {
                ret = false;
                goto _exit;
            }
            merge(graph[i][0], graph[i][j], unionSets);   
        }
    }
...
int main(void)
{
    ...
    int d[2] = {0,2};
    int* graph[4] = {a, b, c, d};
    int graphColSize[4] = {sizeof(a), sizeof(b), sizeof(c), sizeof(d)};
    printf("%d\n", isBipartite(graph, 4, graphColSize));
    return 0;
}
